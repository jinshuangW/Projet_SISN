...
#                    GET DATA                    #
##################################################
align_on =[ 'sample', 'match']
from_time_s = [-500,0,500]
to_time_s = [0,500,1000]
from_time_m = [-500,0]
to_time_m = [0,500]
...
for item in align_on:
    if item == 'sample':
        for j in range(3):

            data_ints = matnpy.get_subset_by_areas(sess_no, raw_path,
                         item, from_time_s[j], to_time_s[j],
                         lowcut, highcut, 
                         areas,
            ...
            for i in range(len(data_ints)):
                targets_prems = [0,0,0,0,0]
                targets_prems[j] = 1
                targets.append(targets_prems)

            data.extend(data_ints)



    else:
        for h in range(2):
            data_intm = matnpy.get_subset_by_areas(sess_no, raw_path,
                                              item, from_time_m[h], to_time_m[h],
                                              lowcut, highcut,
                                              areas,
                                              epsillon=100, order=order,
                                              ...
                                              elec_type=elec_type)
            print(len(data_intm))
            for i in range(len(data_intm)):
                targets_prem = [0, 0, 0, 0, 0]
                targets_prem[h+3] = 1
                targets.append(targets_prem)
            data.extend(data_intm)
...
print(targets)
print(targets.shape)
#targets1 = io.get_targets(decode_for, raw_path, n_chans, elec_type=elec_type,
...
mean_acc= np.mean(mean_acc,axis=0,keepdims= True)
...
df = pd.DataFrame([to_save],
                          columns=['session', 'decode_for', 'only_correct_trials',
                                   'areas', 'cortex', 'accuracy'])
file_name = (base_path + 'results/training/'
                     + sess_no + '_accuracy_' + decode_for + '.csv')
